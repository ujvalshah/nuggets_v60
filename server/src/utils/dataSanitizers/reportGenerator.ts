...
function formatReportAsMarkdown(report: SanitizationReport): string {
  ...
  md += `**Mode:** ${dryRun ? '🔍 DRY RUN (No data modified)' : '⚠️ EXECUTION MODE (Data modified)'}\n\n`;
  md += `---\n\n`;
  ...
  md += `| Category | Count |\n`;
  md += `|----------|-------|\n`;
  md += `| ✅ SAFE_AUTO_FIX | ${summary.byCategory.SAFE_AUTO_FIX} |\n`;
  md += `| ⚠️ CONDITIONAL_CLEANUP | ${summary.byCategory.CONDITIONAL_CLEANUP} |\n`;
  md += `| 🚫 DO_NOT_TOUCH | ${summary.byCategory.DO_NOT_TOUCH} |\n\n`;
  ...
  if (safeAutoFixCount > 0) {
    md += `### ✅ Safe Auto-Fix Issues (${safeAutoFixCount})\n\n`;
  ...
  if (conditionalCount > 0) {
    md += `### ⚠️ Conditional Cleanup Issues (${conditionalCount})\n\n`;
  ...
  if (doNotTouchCount > 0) {
    md += `### 🚫 Do Not Touch Issues (${doNotTouchCount})\n\n`;
