...
export const AddToCollectionModal: React.FC<AddToCollectionModalProps> = ({ 
  ...
  const loadCollections = async () => {
    ...
    try {
      ...
      // Filter by mode and user ownership for private folders
      // For public collections, show all public collections (user can add to any)
      if (mode === 'private') {
        setCollections(cols.filter(c => c.creatorId === currentUserId && c.type === 'private'));
      } else {
  ...
  const createCollection = async () => {
    ...
    try {
      ...
      toast.success(`Created ${mode === 'private' ? 'Private Folder' : 'Community Collection'} and added nuggets`);
    } catch (e) {
  ...
  const title = mode === 'private' ? 'Save to Private Folder' : 'Add to Community Collection';
  const entityName = mode === 'private' ? 'Private Folder' : 'Community Collection';
  const entityNamePlural = mode === 'private' ? 'Private Folders' : 'Community Collections';
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                {searchQuery ? `No ${entityNamePlural.toLowerCase()} found` : `No ${entityNamePlural.toLowerCase()} yet`}
              </p>
            </div>
          ) : (
